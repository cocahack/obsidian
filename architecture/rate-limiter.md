...
그러나 첫 번째 구현 방법은 직전 시간대에 도착한 요청이 균등하게 분포되어 있다는 가정하에 추정치를 계산하므로 실제적으로는 오차가 존재한다는 단점이 있다(그러나 클라우드플레어에 따르면 오차로 인해 버려진 요청은 0.003%에 불과했다고 한다). 두 번째 방법도 처리된 요청 시각을 느슨하게 보고 있어 아니므로 엄격한 처리율 제한이 필요한 경우에는 적합하지 않다.
