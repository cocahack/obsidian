...
그러나 클라이언트와 서버 사이에 컴포넌트가 하나 더 발생하기 때문에 관리 요소가 되며, SPoF가 발생하지 않도록 주의해야 할 것이다.
...
정리하면, 처리율 제한 장치는 클라이언트와 서버 사이에 위치하며 Redis와 같은 카운터 저장소를 이용하여 구현한다. 
...
추가로, 클라이언트에게 자신이 보낸 요청이 처리율 제한에 걸렸고, 
