...
위와 같은 캐시 전략을 read-through caching strategy라고 한다. 그 밖에 다양한 캐시 전략은 [[caching-strategy|캐싱 전략]] 문서에서 설명한다.
...
- Reshading(rebalancing과 같은지?): 데이터가 너무 많아져서 샤드를 늘려야 하거나, 샤드 간 데이터 분포가 고르지 못하여 어떤 샤드의 공간 소모가 빠르게 진행될 때 샤드 키를 계산하는 함수를 변경하고 데이터를 재배치해야 한다. 여기에 [[consistent-hashing|consistent hashing]] 기법을 사용해볼 수 있다.
